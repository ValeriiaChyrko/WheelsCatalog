...
public class UpdateBrandHandlerTests
{
    ...
    public async Task Handle_ValidRequest_ShouldUpdateBrandAndReturnId()
    {
        ...
        _mockBrandRepository.Verify(x => x.UpdateAsync(brand, CancellationToken.None), Times.Once);
    }
    ...
    public void Handle_BrandNotFound_ThrowsNotFoundRequestException()
    {
        ...
        _mockBrandRepository.Setup(x => x.GetByIdAsync(brand.Id.Value, CancellationToken.None))
            .ReturnsAsync((Domain.BrandAggregate.Brand)null!);

        // Act & Assert
        Assert.ThrowsAsync<NotFoundRequestException>(async () =>
            await _handler.Handle(request, CancellationToken.None));
        _mockBrandRepository.Verify(x => x.UpdateAsync(It.IsAny<Domain.BrandAggregate.Brand>(), CancellationToken.None), Times.Never);
    }
